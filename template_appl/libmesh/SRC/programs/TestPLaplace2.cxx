...
int main(int argc, char **argv)
{
  ...
  fDebug.open("testp2_", MPI_COMM_WORLD);
...

  {
    ...
    std::ostringstream s;
    s << "testp2_sol_" << baseName << "_" << rank+1 << suffix;
    MEDLoader::WriteField(s.str().c_str(), sol, true);
    fDebug << "Elapsed (write solution)     : " << T.elapsed() << std::endl;

    T.start();
    MEDCouplingFieldDouble * bdy = P.getValuesOnBoundary(bcNames[0], "u");
    fDebug << "bdy = " << bdy << std::endl;
    fDebug << "Elapsed (get output)         : " << T.elapsed() << std::endl;
    
    if (bdy) {
      std::ostringstream t;
      t << "testp2_bdy_" << baseName << "_" << rank+1 << suffix;
      MEDLoader::WriteField(t.str().c_str(), bdy, true);
      fDebug << "Elapsed (write boundary)     : " << T.elapsed() << std::endl;
      bdy->decrRef();
    }
	...
	: true;
      std::ostringstream s;
      s << "TestPLaplace2: result at node " << n << " ) = " << sol->getIJ(n,0);
      if (good[rank] < 0) 
	s << " unknown";
      else
