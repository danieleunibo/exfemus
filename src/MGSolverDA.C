...
//       _data_eq[deg].mg_eqs.insert (std::pair<int,MGSolBase *>(kl,NULL) );
  ...
  _data_eq[2].indx_ub[0]=DIMENSION; //_data_eq[2].n_eqs=0; // piecewice quadratic (2)

  /// B) List of external equations:
 int n_equations=pbName.size();
  //==================================================================
  // Piecewise constant  -> _data_eq[0])   -----------------------------------------
  //==================================================================
...
for(int iname=0;iname<n_equations;iname++) if(pbName[iname]== K_F){
  ...
  _data_eq[2].mg_eqs[n_index]=_mgeqnmap.get_eqs("K");                // Navier-Stokes equation pointer
  _data_eq[2].indx_ub[n_index+1] =_data_eq[2].indx_ub[n_index]+1; // _data_eq[2].ub index
  _data_eq[2].n_eqs++;                                            // number of quadratic system
  n_index++;          // update counter
  ...
  _data_eq[2].mg_eqs[n_index]=_mgeqnmap.get_eqs("K2");                // Navier-Stokes equation pointer
  _data_eq[2].indx_ub[n_index+1] =_data_eq[2].indx_ub[n_index]+1; // _data_eq[2].ub index
  _data_eq[2].n_eqs++;                                               // number of quadratic system
  n_index++;  // update counter
...
//  end Reading  mat_id ******************************************************
    ...
    for(int iel=0; iel <off_el[off_proc+_NoLevels]-off_el[off_proc+_NoLevels-1]; iel++) {
      ...
      for(int i=0; i<NDOF_FEM; i++) {
        ...
        ic_read(face_id_node,mat_id_elem,xp,u_value);//-----------------------==
        // -------------------------------------------------==
        // ===================================================
    ...
    for(int iel=0; iel <delta; iel++) { // element loop
      ...
      for(int i=0; i< ndof_femv; i++) { // node lement loop
        ...
        for(int ivar=0; ivar<n_dofs; ivar++)  { // variable loop
          ...
          bc_Neu[ivar]=1;  // set 1 all the points for  bc (boundary condition)
        }
...
void MGSolDA::bc_intern_read(
  ...
  /// Default: all Neumann
  for(int ivar=0;ivar<_n_vars;ivar++) {bc_flag[ivar]=1;bc_Neum[ivar]=1;}
  return;
}
// ===========================================================
...
//   const std::vector<double> phi,    // shape functions  <-
//   const std::vector<double> dphi,   // derivatives of the shape functions  <-
  const int
...
//     uold[ivar]=0.;
    for(int
        jdim=0; jdim< DIMENSION; jdim++) uold_dx[ivar*DIMENSION+jdim]=0.;  // set zero
    // interpolation with shape functions
    for(int   eln=0; eln<n_shape; eln++) {
//       const int  indx = ;
//       uold[ivar] += phi[eln]*uold_b[indx];
      for(int
...
void  MGSolDA::interp_el_sol(
  ...
  for(int
      ivar=ivar0; ivar<ivar0+ nvars; ivar++) {
    uold[ivar]=0.;
    for(int
        eln=0; eln<n_shape; eln++)
    ...
    for(int iproc=0; iproc<_mgmesh._n_subdom; iproc++) {
      ...
      for(int iel=0; iel <delta; iel++) {
        ...
        for(int isubcell=0; isubcell<NSUBDOM; isubcell++) {
          sol_p[indx*NSUBDOM+isubcell]=val;
        }
...
//     std::string i_aux_dir = _mgutils.get_file("MESH_AUX_DIR");    // mesh aux dir
    ...
    namefile_mesh<< resu_dir << "/mesh.msh1.h5";
    hid_t  file_mesh = H5Fopen(namefile_mesh.str().c_str(),H5F_ACC_RDWR, H5P_DEFAULT);

    int Level=_NoLevels-1;                                        // Reading from Level
...
void MGSolDA::ic_read(
  ...
  for(int ivar=0; ivar<_n_vars; ivar++) ic[ivar]=25.;
