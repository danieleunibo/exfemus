...
//       // STOP_LOG("send_receive()", "ParallelM");
  ...
  MPI_Pack (&sendsize, 1, ParallelM::StandardType<unsigned int>(),
	    &sendbuf[0], static_cast<int>(sendbuf.size()), &pos,
            comm.get());
  ...
  for (std::size_t i=0; i<send.size(); i++)
    {
      ...
      if (!send[i].empty())
        MPI_Pack (&send[i][0], static_cast<int>(send[i].size()),
  ...
  for (std::size_t i=0; i<recv.size(); i++)
    {
      ...
      if (!recv[i].empty())
        MPI_Unpack (&recvbuf[0], static_cast<int>(recvbuf.size()), &pos,
                    &recv[i][0], static_cast<int>(recv[i].size()),
 ...
 * deprecated - instead of libMesh::ParallelM::Communicator_World use
 * libMesh::CommWorld
 */
...
//   int maxval = tagvalue;
//   this->max(maxval);
//   assert_equal_to (tagvalue, maxval);
// #endif
...
inline void Communicator::min(std::vector<T> &r) const
{
  if (this->size() > 1 && !r.empty())
    {
      // START_LOG("min(vector)", "ParallelM");
  ...
  if (this->size() > 1 && !r.empty())
    {
      ...
      // STOP_LOG("max(vector<bool>)", "ParallelM");
    }
}
                                 ...
                                 unsigned int &max_id) const
{
  if (this->size() > 1)
    {
      // START_LOG("maxloc(bool)", "ParallelM");
      ...
      // STOP_LOG("maxloc(bool)", "ParallelM");
    }
  else
    max_id = this->rank();
}
                                             ...
                                             const MessageTag &tag) const
{
  ...
  ParallelM::pack_range(context, range_begin, range_end, buffer);
...
inline void Communicator::alltoall(std::vector<T> &buf) const
{
  ...
  const int size_per_proc =
    static_cast<int>(buf.size()/this->size());

  assert_equal_to (buf.size()%this->size(), 0);
                                     ...
                                     const unsigned int root_id) const
{
  ...
  const int ierr =
#endif
    MPI_Bcast (data_ptr, static_cast<int>(data.size()),
   ...
   const unsigned int root_id) const
{
  ...
  if (this->rank() == root_id)
    ParallelM::pack_range(context1, range_begin, range_end, buffer);
  ...
  if (this->rank() != root_id)
    ParallelM::unpack_range(buffer, context2, out);
}
...
inline void Communicator::send_receive (const unsigned int send_tgt,
                                        ...
                                        const MessageTag &) const
{
  assert_equal_to (send_tgt, 0);
  assert_equal_to (recv_source, 0);
  recv = send;
}
