...
// =================================================
void MGSolT_ADJ::ic_read(int bc_gam,int bc_mat, double xp[],double u_value[]) {
// =================================================
#if DIMENSION==2
...
void MGSolT_ADJ::bc_read(int bc_gam,int bc_mat, double xp[],int bc_Neum[],int bc_flag[]) {
  // =================================================
...
//   if (xp[1]< LYB*ILref+BDRY_TOLL) {    bc_Neum[0]=0;       bc_flag[0]=2;  }// bottom
  ...
  // cyl
  if (xp[0]> LXE*ILref-BDRY_TOLL) {    bc_Neum[0]=1;       bc_flag[0]=0;  }   // right
  if (xp[1]< LYB*ILref+BDRY_TOLL) {    bc_Neum[0]=0;       bc_flag[0]=0;  }   // bottom
  if (xp[1]> LYE*ILref-BDRY_TOLL) {    bc_Neum[0]=1;       bc_flag[0]=0;  }   // top
  if (xp[0]< LXB*ILref+BDRY_TOLL) {    bc_Neum[0]=0;       bc_flag[0]=0;  }   // left

//   // channel with injection
//   if (bc_gam==10) {bc_Neum[0]=0;       bc_flag[0]=0;} //little injection
//   if (bc_gam==20) {bc_Neum[0]=0;       bc_flag[0]=0;} //big injection (left)
//   if (bc_gam==30) {bc_Neum[0]=0;       bc_flag[0]=0;} //wall
//   if (bc_gam==40) {bc_Neum[0]=0;       bc_flag[0]=0;} //outlet
