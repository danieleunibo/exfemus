...
// ============================================
#if (TBK_EQUATIONS/2==2) // 3D-2D kappa equation
// ============================================
// class files --------------------------------------------------
...
MGSolTBK::MGSolTBK(
  ...
  if (!varname_in.compare("kt")) {_dir=0;_refvalue[0]=_uref*_uref; }          // kappa
  if (!varname_in.compare("wt")) {
    _dir=1;_refvalue[0]=_uref*_uref*_uref/_lref;  // omega
#else
  _var_names[0]="kt";    _refvalue[0]=_uref*_uref; // kappa
  _var_names[1]="wt";    _refvalue[1]=_uref*_uref*_uref/_lref; // omega
#endif
	...
	IRe_eff[0] = _IRe*(1.+_sigma[0]*_mu_turb);//  visc_eff  at g pt ;
        ...
        for (int i=0; i< el_ndof[2]; i++) { // +++++++++++
          ...
          for (int  ivar=0; ivar< _nvars[2]; ivar++)    {
            ...
            if (mode == 1)                {
              FeM(indx)  +=  dtxJxW_g*(
                                _kappa_g[ivar+_dir]*phii_g/_dt  // time
                               + _c_source[ivar+_dir]*phii_g  
			       + _cross*(ivar+_dir)*phii_g   
                             );
            }
...
//                            			 -_c_source[ivar+_dir]*_sP*phij_g*phii_g
...
// #endif
//                            dtxJxW_g* rho*(1-ivar-_dir)*phij_g*phii_g*_c_diss[ivar+_dir]*_kappa_g[0];
...
// 		   val =1.e-0;
// 		  val *=CMU;
// 		  if((1-bc_normal)*bc_rhs == 1) printf("I am here val %lf k_i %lf ydist %f\n",val,k_i,_y_dist);
                  FeM(indx_row) += Ipenalty*( // Dirichlet -> non-homogeneous flag (1??)
                                     bc_normal*(1-bc_rhs)*(kw_i)// old solution
                                     +(1-bc_normal)*bc_rhs*((1-ivar)*0.
                                                       +(ivar)*(val))
                                   );
                }
                // Assemblying Matrix ---------------------------------
                KeM(indx_row,indx_row) += Ipenalty;  // (Dirichlet bc)
              }// i loop
...
//       printf("c_source is %f and %f, _c_diss is %f and %f and cross is %f and F1 %f and F2 %f \n",_c_source[0],_c_source[1],_c_diss[0],_c_diss[1],_cross,F1,F2);
      ...
      if(_mu_turb*_IRe < nut2) {_c_source[0] =0.5*_mu_turb*_IRe*_sP; _c_source[1] =0.5*13.*_sP/(25.);}
      else {_c_source[0] =0.5*nut2*_sP; _c_source[1] =0.5*13.*nut2*_sP/(25.*_mu_turb*_IRe);}
      
      _c_diss[0]   =  BETASTAR/tau_k;
      _c_diss[1]   = 0.0708/tau_k;                  //ONLY in 2D!!
      if(_chi_k > 0) _cross = _chi_k*tau_k/8.;
