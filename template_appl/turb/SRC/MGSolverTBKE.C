...
// ============================================
#if (TBK_EQUATIONS/2==1) // 3D-2D kappa equation
// ============================================
...
//  ======================================================================
  void  MGSolTBK::GenMatRhs(const double time, const int
	...
	mod2_vel =1.e-20;    // velocity field
        ...
        f_mu(val_tbg);  
        IRe_eff[0] = _IRe*(1.+SIGMA_K*_mu_turb);//  visc_eff  at g pt ;
        IRe_eff[1] = _IRe*(1.+SIGMA_E*_mu_turb);//  visc_eff  at g pt ;
...
//                                               + (IRe_eff[ivar+_dir]+f_upwind*vel_g[ivar+_dir]*vel_g[ivar+_dir])
//                                               *(dphijdx_g[2][ivar+_dir]*dphiidx_g[2][ivar+_dir])        // viscous tensor
 ...
 #endif
//                             dtxJxW_g* rho*(IRe_eff[ivar+_dir]+f_upwind*vel_g[_dir]*vel_g[ivar+idimp1])*
//                             dphijdx_g[2][_dir]*dphiidx_g[2][idimp1];
//                           (1-ivar+_dir)*dtxJxW_g*rho*phij_g*phii_g*_c_diss[ivar+_dir]+	
// 			    (ivar+_dir)*dtxJxW_g*phij_g*phii_g*_c_diss[ivar+_dir];
			     dtxJxW_g*phij_g*phii_g*_c_diss[ivar+_dir];
...
//    double f_e=1.;
    ...
    _c_source[1] =C1O*CMU*_c_source[0]/tau_k; //*exp(-_kappa_g[1]);  // standard kappa-omega model
    _c_diss[0]=1.;      //_kappa_g[1]/_kappa_g[0];//*exp(_kappa_g[0]);
    _c_diss[1]=f_e*C2O*CMU*CMU/(tau_k*tau_k);//*exp(_kappa_g[1]);
