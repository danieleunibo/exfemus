...
/// specified by \p solver_package
    ...
    virtual std::auto_ptr<NumericVectorM > zero_clone () const = 0;
...
//
    ...
    virtual int compare (const NumericVectorM &other_vector,
                         const double threshold = TOLERANCE) const;
    /// @returns \p -1 when \p this is equivalent to \p other_vector,
    ...
    /// threshold.  When no threshold is given, the \p libMesh\p TOLERANCE is used.
