...
NumericVectorM::build(
  ...
  switch (solver_package){
	...
	std::auto_ptr<NumericVectorM > ap(new LaspackVectorM(comm, AUTOMATICM));
	return ap;
      }
#endif
#ifdef HAVE_PETSCM
    case PETSC_SOLVERSM:{
	std::auto_ptr<NumericVectorM > ap(new PetscVectorM(comm, AUTOMATICM));
	return ap;
      }
#endif
// #ifdef LIBMESH_HAVE_TRILINOS
//     case TRILINOS_SOLVERSM:{
// 	std::auto_ptr<NumericVectorM > ap(new EpetraVector<double>(comm, AUTOMATICM));
// 	return ap;
//       }
// #endif
    default:
