...
MGSolTBKA::MGSolTBKA(
  ...
  if(!varname_in.compare("ka")) _dir=0;  // kappa
  if(!varname_in.compare("ea")) _dir=1;  // omega
  _var_names[0]=varname_in;   _refvalue[0]=_uref;
#else
  _var_names[0]="ka";    _refvalue[0]=_uref; // kappa
  _var_names[1]="ea";    _refvalue[1]=_uref; // omega
#endif
...
// derivatives
	...
	val_tbg[1]= 0.;
        for(int idim=0; idim< DIMENSION; idim++) {
          for(int jdim=0; jdim< DIMENSION; jdim++)   // adjoint turbulence source --
            val_tbg[1] += -vel_dxg[idim+jdim*DIMENSION]*velad_dxg[idim+jdim*DIMENSION];
	  val_tbg[1] += -k_dxg[idim]*kad_dxg[idim];
	}
...
//              int bc_normal=(bc_s%2);    // (??1) normal
                ...
                if(mode == 1)  {
                  double kw_i=_data_eq[2].ub[kad_idx*NDOF_FEM+indx_sol];
		  if(kw_i <1.e-20) kw_i =1.e-20;
                  double k_i=fabs(_data_eq[2].ub[kad_idx*NDOF_FEM+sur_toply[i]]);
...
//                     KeM(indx_row,sur_toply[j]+(ivar+_dir)*el_ndof[2]) += dtJxW_g*alpha_t*phij_g*phii_g*bc_normal; // (??1) -> normal
                    KeM(indx_row,sur_toply[j]+ivar*el_ndof[2]) += 2.*IRe_eff[ivar+_dir]*dtJxW_g*phij_g*phii_g*bc_normal/_y_dist;// (??1) -> normal
                  }
                }// i-loop
...
/// This function controls the assembly and the solution of the NS_equation system:
  void MGSolTBKA::MGTimeStep(const double time, const int
    ...
    /// B) Assemblying of the rhs (top level in surface and volume with MGSolTBKA::GenRhs,GenRhsB),
#if PRINT_TIME==1
    std::clock_t start_time=std::clock();
#endif
    GenMatRhs(time,_NoLevels-1,1);
    /// C) Assemblying of the  MGmatrices (MGSolTBKA::GenMatrix),
    ...
    std::cout << "  Assembly time -----> ="<< double(end_time- start_time) / CLOCKS_PER_SEC
              << "s "<< std::endl;
#endif
// //     / E) Solution of the linear MGsystem (MGSolTBKA::MGSolve).
     MGSolve(1.e-9,40);
#if PRINT_TIME==11
    end_time=std::clock();
    std::cout << " Assembly+solution time -----> ="<< double(end_time- start_time) / CLOCKS_PER_SEC
              << "s "<< std::endl;
#endif
    /// F) Update of the old solution at the top Level  (MGSolTBKA::OldSol_update),
...
//     _c_source[1] =CMU*_c_source[0]/tau_k; //*exp(-_kappa_g[1]);  // standard kappa-omega model
    _c_source[0] = 0.5*l_mix*val1[1]/sqrt(_kappa_g[0]);
    _c_diss[0]=   0.12*sqrt(_kappa_g[0])/l_mix+0.5*l_mix*val1[0]/sqrt(_kappa_g[0]);    
