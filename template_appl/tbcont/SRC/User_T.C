...
//   // cyl
  if (xp[0]> LXE*ILref-BDRY_TOLL) {    bc_Neum[0]=1;       bc_flag[0]=0;  }   // right
  if (xp[1]> LYE*ILref-BDRY_TOLL) {    bc_Neum[0]=1;       bc_flag[0]=0;  }   // top
  if (xp[0]< LXB*ILref+BDRY_TOLL) {    bc_Neum[0]=0;       bc_flag[0]=4;  }   // left
  if (xp[1]< LYB*ILref+BDRY_TOLL) {    bc_Neum[0]=0;       bc_flag[0]=1;  }   // bottom
//   if (xp[1]< LYB*ILref+BDRY_TOLL && xp[0]> LXE*ILref-BDRY_TOLL) {    bc_Neum[0]=0;       bc_flag[0]=4;  }   // bottom right
//   if (xp[1] < LYB*ILref + BDRY_TOLL && xp[0] > 1. && xp[0] <2.) {bc_Neum[0]=0; bc_flag[0]=1;} //  injection

//     // channel with injection
//   if (bc_gam==10) {bc_Neum[0]=0;       bc_flag[0]=2;} //little injection
//   if (bc_gam==20) {bc_Neum[0]=0;       bc_flag[0]=4;} //big injection (left)
//   if (bc_gam==30) {bc_Neum[0]=0;       bc_flag[0]=2;} //wall
