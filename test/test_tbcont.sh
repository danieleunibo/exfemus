...
cp -r RESU RESU-coup
sed -i 's/#define T_COUP_EQUATIONS (1)/#define T_COUP_EQUATIONS (2)/g' DATA/Equations_conf.h
make -j4
mpiexec   -np 4 $FM_MYAPP-opt
cp -r RESU RESU-coupDG
sed -i 's/#define T_COUP_EQUATIONS (2)/#define T_COUP_EQUATIONS (1)/g' DATA/Equations_conf.h
mv DATA/Equations_conf_segh DATA/Equations_conf.h
mv DATA/parameters_seg.in DATA/parameters.in
make src_clean
make -j4
mpiexec   -np 4 $FM_MYAPP-opt