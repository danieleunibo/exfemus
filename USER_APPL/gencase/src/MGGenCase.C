...
MGGenCase::MGGenCase(
  ...
  _n_subdomains = libMesh::n_processors();
...
void MGGenCase::GenCase() {
  ...
  BoundaryMesh *bd_msh0= new  BoundaryMesh(msh0->mesh_dimension()-1);
  msh0->boundary_info->sync(*bd_msh0);
  BoundaryMesh *bd_msht= new  BoundaryMesh(msht->mesh_dimension()-1);
  ...
  ///              PrintMesh
  // ********************************************************
  if(libMesh::processor_id() == 0) printMesh(*bd_msht,*msht,*bd_msh0,*msh0,mesh_map_in);
...
void MGGenCase::compute_matrix(
  ...
  for(int Level1 = 0; Level1 < _n_levels; Level1++) {
    ...
    for(int kdom=0; kdom <_n_subdomains; kdom++) {  // piecewise
      nodes_top[2] +=off_el[kdom*_n_levels+Level1+1]-off_el[kdom*_n_levels+Level1];
    }
    ...
    print_op_h5(name.str(),nodes_top[iql],nodes_top[2],count_q,Mat_q,NULL,
                len_q,len_qoff,qlt);
    ...
    count_q=compute_mat_kq(Level1,level_row[iql],level_cln[jql],el_nodes[2], el_nodes[jql],
                           ...
                           v_el,elem_sto,v_inv_nd,
                           g_indexL,off_nd[iql],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[2],nodes_top[iql],count_q,Mat_q,NULL,
                len_q,len_qoff, qlt);
    ...
    count_q=compute_mat_kk(Level1,level_row[iql],level_cln[jql],el_nodes[2],el_nodes[2],
                           ...
                           v_el,elem_sto,v_inv_nd,
                           g_indexL,off_nd[iql],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[2],nodes_top[2],count_q,Mat_q,NULL,
                len_q,len_qoff, qlt);
...
// med->libmesh map  second order only
  ...
  for(; it_tr != end_tr; ++it_tr) {
    ...
    if(lev==Level) {
      for(int  s=0; s<(int) elem->n_sides(); s++) {
        if(elem->neighbor(s) == NULL)    {
          AutoPtr<Elem> side(elem->build_side(s));  // face element
