...
#include "NS_Problem.h"
#include "NS_Problem3D.h"
#include "LinearElasticProblem.h"
#include "Con_DiffProblem.h"
#include "MonodimProblem.h"
...
void LIBMESH::setMesh(const std::string & dataFile) {
...
// ===================================================================
std::vector<std::string> LIBMESH::getBoundaryNames(
) { //================================================================
...
std::string LIBMESH::getBoundaryName(
  int i) {
  if (i>=0 && i<_bc.size())
    return _bc[i].name;
  return "";
...
void LIBMESH::setAnalyticBoundaryValues(const std::string & bcName,
...
void LIBMESH::setFieldBoundaryValues
...
ParaMEDMEM::MEDCouplingFieldDouble * LIBMESH::getValuesOnBoundary_nodes
(const std::string & nameBC,  std::vector<char *> vName) const {
  int id = searchBoundaryCondition(nameBC);
  return _problem ? _problem->getValuesOnBoundary_nodes(id, vName) : NULL;
}

int LIBMESH::searchBoundaryCondition(const std::string & nameBC) const {
...
void LIBMESH::setAnalyticSource(
...
// ===================================================================
// This function sets a numerical source (coupling)
void LIBMESH::setAverageSources(
...
ParaMEDMEM::MEDCouplingFieldDouble *LIBMESH::getOutputField(
...
ParaMEDMEM::MEDCouplingFieldDouble * LIBMESH::getInputFieldTemplate(const std::string &name) {
//    if (name == "")
