...
// ====================================================
#if (TTBK_EQUATIONS/2==2) // 3D-2D kappa-omega equation
// ====================================================
...
MGSolTTBK::MGSolTTBK(
  ...
  _var_names[1]="wh";    _refvalue[1]=_uref*_lref; // omega
#endif
  ...
  _c_source[0]=0.; _c_source[1]=0.; 
  _turb_source[0]=0.; _turb_source[1]=0.;
  _c_diss[0]=0.; _c_diss[1]=0.;
  return;
}
...
//  ======================================================================
  void  MGSolTTBK::GenMatRhs(const double time, const int
    ...
    double T_dxg[DIMENSION]; double KT_dxg[DIMENSION]; double WH_dxg[2*DIMENSION];
...
// 	               360./(10340.*145.75*V_MID*0.03025); //plane
		       2.*360./(10340.*145.75*V_MID*0.03025); //cyl
...
#endif  // ------------------------------------------------------------------------------
        // SST cross diffusion term (Dkt*Dwh)
	int nTURB_idx=_data_eq[2].indx_ub[_data_eq[2].tab_eqs[K_F]];
        interp_el_gdx(_data_eq[2].ub,nTURB_idx,2,_dphi_g[2],el_ndof[2],WH_dxg);
        interp_el_gdx(_data_eq[2].ub,kT_idx,1,_dphi_g[2],el_ndof[2],KT_dxg);
	
	double SST_omega=0.;
	for(int idim=0; idim< DIMENSION; idim++) {
	  SST_omega += KT_dxg[idim]*WH_dxg[idim+DIMENSION];
	}
        
	_kappaT_g[0]= _ub_g[2][kT_idx]; _kappaT_g[1]=  _ub_g[2][kT_idx+1];
        f_mu(val_tbg);
	
	SST_omega *= (2./_kappaT_g[0])*(_IPrdl*_IRe+_nut_ratio*_IRe*_IPrdl_turb*SIGMA_WH);
// 	printf("SST omega is %f \n",SST_omega);      


        /// d) Assemblying Kt-Wt equation
	    ...
	    if(ivar==1)   alpha_eff = _IPrdl*_IRe+_nut_ratio*_IRe*_IPrdl_turb*SIGMA_WH;
            ...
            if(mode == 1)                {
              FeM(indx)  +=  dtxJxW_g*(
                                  _kappaT_g[ivar+_dir]*phii_g/_dt    // _ub_g[2][kT_idx+ivar+_dir]*phii_g/_dt   // time
                                  +_c_source[ivar+_dir]*_sT*phii_g
                                  +(ivar+_dir)*_turb_source[ivar+_dir]*phii_g  // mechanical source for omega
// 				  + SST_omega*(ivar+_dir)*phii_g      //!!!!!!!!!!! SST-like term in omega equation
...
// 	      int idimp1=(ivar+_dir)%2; // block +1 [2-6-7] ---------------
...
// 		  double val=0;//.400772603053*k_i*sqrt(k_i)/(_y_dist);
		   val=6.*_muf/(_rhof*BETASTAR*_y_dist*_y_dist);//sqrt(k_i)/(BETASTAR*_y_dist)
...
//  		  if(val >2000) val =2000;
            ...
            //  Neumann boundary conditions  ***********************************
              ...
              for(int qp=0; qp<  elb_ngauss; qp++) {  //gaussian integration loop (n_gauss)
                ...
                det[2]     = _fe[2]->JacSur(qp,xxb_qnds,InvJac[2]);   // local coord _nut_ratio_phi_g and jac
...
//       double alpha_star        = 1.;//(0.024+ Re_t/6.)/(1.+ Re_t/6.);
      ...
      if (alpha_starb < 1.) {alpha_starb=1.;} //else {printf("Using alpha_starb %f \n",alpha_starb);}
...
// 	if(_kappaT_g[1]< 1.e-20) _kappaT_g[1]= 1.e-5;           // kappa
	if(_kappaT_g[0]< 1.e-6) _kappaT_g[0]= 1.e-6;           // kappa
...
//     double tauT_k= CMU*_kappaT_g[0]/_kappaT_g[1];
 ...
 double tauT_k=1./_kappaT_g[1]; // tau=1/omega
  #endif
  if (tauT_k> MAX_TAUT) {tauT_k= MAX_TAUT; /*printf("Maximum TAUT reached \n");*/}  // tau=1/omega=CMU*kappa/epsilon
  if (tauT_k< MIN_TAUT) {tauT_k= MIN_TAUT; /*printf("Minimum TAUT reached \n");*/}  // tau=1/omega=CMU*kappa/epsilon
    double rT=tauT_k/tau_k;
//   if (rT<0.025) rT=0.025;
//   if (rT>3.) rT=3.;
...
//       double f_asym =(1.-exp(-1.*R_eps/(10.*sqrt(_IPrdl)) ))*(1.-exp(-1.*R_eps/10.));
...
//       double cd2var =0.9;//(1.9*(1.-0.3*exp(-R_t*R_t/(6.5*6.5)))-1.)*(1.-exp(-R_eps/5.7))*(1.-exp(-R_eps/5.7));//0.025*sqrt(R_t)*(1+3.*sqrt(1/(_IPrdl*rT))));
...
// 
//     _c_diss[0]=_kappaT_g[1];
//     _c_diss[1]=CD1*CMU*CMU/(tauT_k*tauT_k);//_kappaT_g[1]/_kappaT_g[0];
// //  		+ cd2var*CD2*CMU/tau_k // mechanical dissipation for epsilon
	 
//     double cd2var =(1.9*(1.-0.3*exp(-R_t*R_t/(6.5*6.5)))-1.)*(1.-exp(-R_eps/5.7))*(1.-exp(-R_eps/5.7));
//     cd2var=0.9;
    _c_source[0]=_nut_ratio*_IRe*_IPrdl_turb;
    _c_source[1] =(CP1-1.)*_c_source[0]/(_kappaT_g[0]*tauT_k);
    
    _turb_source[0] =CD2*CMU/tau_k;
    _turb_source[1] =CP2*_sP/(rT);

    _c_diss[0]=CMU/tauT_k;
    _c_diss[1]=(CD1-1.)*CMU/(tauT_k);

//     printf("c_source is %f and %f, _turb_source is %f and %f, _c_diss is %f and %f \n",
// 	   _c_source[0],_c_source[1],_turb_source[0],_turb_source[1],_c_diss[0],_c_diss[1]);
    
//     if(_turb_source[0]>1.e+3) _turb_source[0]=1.e+3;
//     if(_turb_source[1]>1.e+3) _turb_source[1]=1.e+3;
//     if(_c_diss[0]>1.e+3) _c_diss[0]=1.e+3;
//     if(_c_diss[1]>1.e+3) _c_diss[1]=1.e+3;
//     if(_turb_source[0]<-1.e+3) _turb_source[0]=-1.e+3;
//     if(_turb_source[1]<-1.e+3) _turb_source[1]=-1.e+3;
//     if(_c_diss[0]<-1.e+3) _c_diss[0]=-1.e+3;
//     if(_c_diss[1]<-1.e+3) _c_diss[1]=-1.e+3;
