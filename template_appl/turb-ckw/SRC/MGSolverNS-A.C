...
// ===============================================================
  ...
  double velad_dxg[DIMENSION*DIMENSION]; double k_dxg[DIMENSION*2]; double kad_dxg[DIMENSION*2];
  double kappa_mg[2];
...
// #endif
	...
	interp_el_gdx(_data_eq[2].ub,k_idx,2,_dphi_g[2],el_ndof[2],k_dxg);
	interp_el_gdx(_data_eq[2].ub,kad_idx,2,_dphi_g[2],el_ndof[2],kad_dxg);  
...
//       IRe_eff = _IRe;
	...
	for (int init=0; init< ndim; init++) {adj_source[init]=0.; adj_source2[init]=0.;}
	  ...
	  const double multi=2.*(_ub_g[2][kad_idx]*_mu_turb*_IRe+13.*_ub_g[2][kad_idx+1]/25.);
	  for (int idim=0; idim < ndim; idim++) {
...
// 	    adj_source[jvar]  += -_ub_g[2][nsad_idx+idim+_dir]*vel_gdx[idim+jvar*DIMENSION];  //advection only for adjoint (explicit)
	  }
	  adj_source[jvar]+= -_ub_g[2][kad_idx]*k_dxg[jvar]-_ub_g[2][kad_idx+1]*k_dxg[jvar+ndim];
        }
// 	  adj_source[0] -= vel_g[0]-0.;
// 	  adj_source[1] -= vel_g[1]-1.;
// 	  adj_source[2] -= vel_g[2]-0.1;
        ...
        for (int  ivar=0; ivar< _nvars[2]; ivar++)    {
          ...
          if (mode == 1)                {
            FeM(indx)  +=  dtxJxW_g*(
                             rho*_ub_g[2][nsad_idx+ivar+_dir]*phii_g/dt     // time
//                              + rho*_IFr*_dirg[ivar+_dir]*phii_g  // x-gravity
          ...
          for (int j=0; j<el_ndof[2]; j++) {
            ...
            KeM(indx,j+ivar*el_ndof[2]) +=dtxJxW_g*rho*(
                                            ...
                                            + phii_g*vel_gdx[ivar+ivar*DIMENSION]*phij_g    // advection only for adjoint (implicit)
...
#endif
                         dtxJxW_g*rho*(
			    phii_g*vel_gdx[idimp1+ivar*DIMENSION]*phij_g +    // advection only for adjoint (implicit)
