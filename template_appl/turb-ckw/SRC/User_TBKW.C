...
#else               // --------- coupled ---------------------------
  // xp[]=(xp,yp,zp)   kappa -> 0  omega -> 1(0.002)
//   u_value[0]= 0.;
//   u_value[1] =0.;
//   if(xp[1]<0.01) {
//     if(xp[0] >LXB*ILref+BDRY_TOLL && xp[0] <LXE*ILref-BDRY_TOLL) { u_value[0] =1.+ 0*(xp[0] -LXB*ILref)*(LXE*ILref-xp[0])/_uref; }
//   }
   u_value[0] = .1;
  u_value[1] = 1.;
...
// cyl
  ...
  if(xp[1] < LYB*ILref + BDRY_TOLL) {bc_flag[0]=1; bc_flag[1]=1; bc_Neum[0]=0; bc_Neum[1]=0;} //  bottom
  if(xp[0] > LXE*ILref - BDRY_TOLL) {bc_flag[0]=1; bc_flag[1]=2; bc_Neum[0]=1; bc_Neum[1]=0;} //  right
// annulus
//   if(xp[1] > LYE*ILref - BDRY_TOLL) {bc_flag[0]=0; bc_flag[1]=0; bc_Neum[0]=1; bc_Neum[1]=1;} //  top
...
//  tri bundle
//    bc_flag[0]=0;bc_Neum[0]=1;bc_flag[1]=0;bc_Neum[1]=1;
//  if (xp[1]*xp[1]+(xp[0]-0.00615)*(xp[0]-0.00615)< 0.0041*0.0041+BDRY_TOLL){               //rod arc
//      bc_flag[0]=0;bc_Neum[0]=0;bc_flag[1]=0;bc_Neum[1]=0;}
