...
int main(int argc, char **argv) {
  ...
  std::string dataFile1 =  "MESH/line.med"; // Initial dataFile
...
//   // 4. set source  ->  P.setAnalyticSource("0")
  ...
  P1.setNAverageSources(1);  // for 1 constant average value
...
//   }
    // for constant average value from P
    MEDCouplingFieldDouble *bdy=NULL;
    std::vector<char*> name;
    name.push_back("u");
    bdy = P.getValuesOnBoundary("share", "u");
    double max=  bdy->getMaxValue();
    double average[1]; average[0] = bdy-> getAverageValue();
    double average_wei= bdy-> getWeightedAverageValue();
    double min=  bdy->getMinValue();
    // constant average value set to bc in P1
    P1.setAverageSources(average);
    ...
    if(itime%1==0) {
      ...
      std::string s =  "RESU/convdiff_sol_";  s += prefix;  s += ".med";
      std::string s1 = "RESU/convdiff_sol_";  s1 += prefix1;  s1 += ".med";
      std::cout << " File "<< s <<"\n"; std::cout << " File "<< s1 <<"\n";
      std::ostringstream  s2; s2<<"RESU/convdiff_sol_" << prefix<<"."<<itime<<".med";
      std::ostringstream  s3; s3<<"RESU/convdiff_sol_" << prefix1<<"."<<itime<<".med";
