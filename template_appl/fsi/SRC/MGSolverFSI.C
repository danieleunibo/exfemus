...
double Compute_J(double  _ub_dxg[DIMENSION*DIMENSION] ){
  	    #if DIMENSION==2
                double Jloc= (1+_ub_dxg[0+0*DIMENSION])*(1+_ub_dxg[1+1*DIMENSION])-( _ub_dxg[1+0*DIMENSION]*_ub_dxg[0+1*DIMENSION]);
           #endif
           #if DIMENSION==3
                double Jloc= (
                              (1+_ub_dxg[0+0*DIMENSION])*(1+_ub_dxg[1+1*DIMENSION])*(1+_ub_dxg[2+2*DIMENSION])
                              +  _ub_dxg[0+1*DIMENSION]* _ub_dxg[1+2*DIMENSION]* _ub_dxg[2+0*DIMENSION]
                              +  _ub_dxg[0+2*DIMENSION]* _ub_dxg[1+0*DIMENSION]* _ub_dxg[2+1*DIMENSION]
                          )
                          -(
                              _ub_dxg[2+0*DIMENSION]*(1+_ub_dxg[1+1*DIMENSION])*_ub_dxg[0+2*DIMENSION]
                              +  _ub_dxg[2+1*DIMENSION]*_ub_dxg[1+2*DIMENSION]* (1+_ub_dxg[0+0*DIMENSION])
                              + (1+_ub_dxg[2+2*DIMENSION]) *_ub_dxg[1+0*DIMENSION]* _ub_dxg[0+1*DIMENSION]

                          );
#endif

return (Jloc) ;}
double Compute_I(double  _ub_dxg[DIMENSION*DIMENSION] ){
  	    #if DIMENSION==2
                double Iloc= pow((_ub_dxg[0+0*DIMENSION]+1),2)+ pow((_ub_dxg[0+1*DIMENSION]),2)+
                          pow((_ub_dxg[1+1*DIMENSION]+1),2)+ pow((_ub_dxg[1+0*DIMENSION]),2); //first invariant of the trasformation
           #endif
           #if DIMENSION==3
                double Iloc= pow((_ub_dxg[0+0*DIMENSION]+1),2)+ pow((_ub_dxg[0+1*DIMENSION]),2)+pow((_ub_dxg[0+2*DIMENSION]),2)+
                          pow((_ub_dxg[1+1*DIMENSION]+1),2)+ pow((_ub_dxg[1+0*DIMENSION]),2)+ pow((_ub_dxg[1+2*DIMENSION]),2)+ //first invariant of the trasformation
                          pow((_ub_dxg[2+2*DIMENSION]+1),2)+ pow((_ub_dxg[2+0*DIMENSION]),2)+ pow((_ub_dxg[2+1*DIMENSION]),2) ;
#endif

return (Iloc) ;}
// ====================================================================
/// This function assembles the quadratic term of cauchy tensor:
//  ===================================================================
double Compute_quad_term(double  _ub_dxg[DIMENSION*DIMENSION] , int ivar, double mus, double dphiidx_g[3][DIMENSION], int _dir, int ndim, double I, double J){
  double quad_term=0;
   int idimp1=(ivar+1+_dir)%ndim; // block +1 [2-6-7] ---------------
                     ...
                     int idimp2=(ivar+2+_dir)%ndim; // block +1 [2-6-7] ---------------
                            ...
                            quad_term=mus/J*(
  dphiidx_g[2][ivar]*(
                                                    +_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[ivar+DIMENSION*ivar]
                                                ...
                                                dphiidx_g[2][idimp1]*(
                                                    +_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[idimp1+DIMENSION*ivar]
 ...
 +dphiidx_g[2][ivar]*(
			    ...
			    -3*(
			     +2*_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[ivar+DIMENSION*ivar]
			     + _ub_dxg[ivar+DIMENSION*idimp1]*_ub_dxg[ivar+DIMENSION*idimp1]
			     #if DIMENSION==3
			     +_ub_dxg[ivar+DIMENSION*idimp2]*_ub_dxg[ivar+DIMENSION*idimp2]
			     #endif
			    ...
			    -(
			      +_ub_dxg[idimp1+DIMENSION*ivar]*_ub_dxg[idimp1+DIMENSION*ivar]
			       #if DIMENSION==3
			      +_ub_dxg[idimp2+DIMENSION*ivar]*_ub_dxg[idimp2+DIMENSION*ivar]
			      #endif
			    )
			    -2*(
			      +_ub_dxg[ivar+DIMENSION*idimp1]*_ub_dxg[idimp1+DIMENSION*ivar]
			      #if DIMENSION==3
			       +_ub_dxg[ivar+DIMENSION*idimp2]*_ub_dxg[idimp2+DIMENSION*ivar]
			      #endif
			    )
)
  +dphiidx_g[2][idimp1]*(
...
-2*(_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[ivar+DIMENSION*idimp1])
-4*(_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[idimp1+DIMENSION*ivar])
-4*(_ub_dxg[ivar+DIMENSION*idimp1]*_ub_dxg[idimp1+DIMENSION*idimp1])   
-2*(_ub_dxg[idimp1+DIMENSION*ivar]*_ub_dxg[idimp1+DIMENSION*idimp1])
 #if DIMENSION==3
-3*(_ub_dxg[ivar+DIMENSION*idimp2]*_ub_dxg[idimp1+DIMENSION*idimp2])
-(_ub_dxg[idimp1+DIMENSION*idimp2]*_ub_dxg[idimp2+DIMENSION*ivar])
-_ub_dxg[idimp2+DIMENSION*idimp1]*(_ub_dxg[idimp2+DIMENSION*ivar]+_ub_dxg[ivar+DIMENSION*idimp2])
  #endif
  )
  )
 #if DIMENSION==3
  +dphiidx_g[2][idimp2]*(
    ...
    +(
 -2*(_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[ivar+DIMENSION*idimp2])
-4*(_ub_dxg[ivar+DIMENSION*ivar]*_ub_dxg[idimp2+DIMENSION*ivar])
-4*(_ub_dxg[ivar+DIMENSION*idimp2]*_ub_dxg[idimp2+DIMENSION*idimp2])   //
-2*(_ub_dxg[idimp2+DIMENSION*ivar]*_ub_dxg[idimp2+DIMENSION*idimp2])// 
-3*(_ub_dxg[ivar+DIMENSION*idimp1]*_ub_dxg[idimp2+DIMENSION*idimp1])
-(_ub_dxg[idimp2+DIMENSION*idimp1]*_ub_dxg[idimp1+DIMENSION*ivar])
-_ub_dxg[idimp1+DIMENSION*idimp2]*(_ub_dxg[idimp1+DIMENSION*ivar]+_ub_dxg[ivar+DIMENSION*idimp1])
 )
    )
  #endif
...
#else
            h_eff[idim]=fabs(xx_qnds[idim*NDOF_FEM+1]-xx_qnds[idim*NDOF_FEM+0]);
            double h_1=fabs(xx_qnds[idim*NDOF_FEM+2]-xx_qnds[idim*NDOF_FEM+1]);
             if (h_eff[idim] <  h_1) h_eff[idim]=h_1; // Max dx diagonal term
              h_1=fabs(xx_qnds[idim*NDOF_FEM+2]-xx_qnds[idim*NDOF_FEM+0]);
             if (h_eff[idim] <  h_1) h_eff[idim]=h_1; // Max dx diagonal term
#if DIMENSION==3
             h_1=fabs(xx_qnds[idim*NDOF_FEM+3]-xx_qnds[idim*NDOF_FEM+0]);
             if (h_eff[idim] <  h_1) h_eff[idim]=h_1; // Max dx diagonal term
            h_1=fabs(xx_qnds[idim*NDOF_FEM+3]-xx_qnds[idim*NDOF_FEM+1]);
             if (h_eff[idim] <  h_1) h_eff[idim]=h_1; // Max dx diagonal term
              h_1=fabs(xx_qnds[idim*NDOF_FEM+3]-xx_qnds[idim*NDOF_FEM+2]);
             if (h_eff[idim] <  h_1) h_eff[idim]=h_1; // Max dx diagonal term
#endif
#endif
        }
...
// #endif
        ...
        kappa_mg[1] =0;
        mu_m =0.;
//      for(int i=0; i<NDOF_FEM; i++)  if(_bc_vol[i] > 3.5 && _bc_vol[i] < 9.5)  ls_interface=1.;
...
//             interp_el_gdx(_data_eq[2].ub,ns_idx,DIMENSION,_dphi_g[2],el_ndof[2],vel_gdx); // derivatives
		...
		_ale_vel_g[idim]=(_disp_g[idim]/_dt); //ale velocity
                ...
                for (int jdim=0; jdim< DIMENSION; jdim++) {
                    const double sss=0.5*(vel_gdx[idim+jdim*DIMENSION]+vel_gdx[jdim+idim*DIMENSION]);
                    _sP += 2.*sss*sss;
                }
            }
...
//        mu_m +=IRe_eff;
            ...
            for (int i=0; i< el_ndof[2]; i++) { // +++++++++++
                ...
                for (int  ivar=0; ivar< _nvars[2]; ivar++)    {
                    ...
                    double dtxJxW_g=JxW_g[2]*(_bc_vol[i+ivar*NDOF_FEM]%2);
...
#endif
                                     dtxJxW_g* rho*(IRe_eff+0/*0.*f_upwind[ivar+_dir]*vel_g[ivar+_dir]*vel_g[idimp2]*/)*
...
#endif
// 			    J=1;
                            sig_1_lin[ivar]=_mus/J*(2*dphijdx_g[2][ivar]*dphiidx_g[2][ivar]+
                                                                 dphijdx_g[2][idimp1]*dphiidx_g[2][idimp1]
#if DIMENSION==3
                                                                 + dphijdx_g[2][idimp2]*dphiidx_g[2][idimp2]
#endif
                                                                );
                            sig_1_lin[idimp1]= _mus/J*(dphijdx_g[2][ivar+_dir]*dphiidx_g[2][idimp1]);
#if DIMENSION==3

                            sig_1_lin[idimp2]= _mus/J*(dphijdx_g[2][ivar]*dphiidx_g[2][idimp2])
#endif
                                               ;
#ifdef COMPRESSIBLE
...
/// Linear tensor
                            ...
                            sig_1_lin[ivar]=2*c1*(2*dphijdx_g[2][ivar]*dphiidx_g[2][ivar]+
                                                                 dphijdx_g[2][idimp1]*dphiidx_g[2][idimp1]
#if DIMENSION==3
                                                                 + dphijdx_g[2][idimp2]*dphiidx_g[2][idimp2]
#endif
                                                                );
                            sig_1_lin[idimp1]= 2*c1*(dphijdx_g[2][ivar+_dir]*dphiidx_g[2][idimp1]);
#if DIMENSION==3

                            sig_1_lin[idimp2]= 2*c1*(dphijdx_g[2][ivar]*dphiidx_g[2][idimp2])
#endif
                                               ;
...
//                         int idimp2=(ivar+2+_dir)%ndim; // block +2 [3-4-8] ------------
		     ...
		     FeM(indx)+=-_data_eq[2].ub[(sdx_idx)*NDOF_FEM+j+idimp2*el_ndof[2]]*//          _u_old[j+idimp2*el_ndof[2]]*
                                         dtxJxW_g* rho*(
                                             sig_1_lin[idimp2]+sig_3_lin[idimp2]  
                                         );
#endif                // ----------------------------------------------------------------------
                    } // end A element matrix quad -quad (end loop on j)---------------------------------------------
